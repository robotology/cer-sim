...
def generate_launch_description():
    ...
    robot_xacro_decl = DeclareLaunchArgument(
            'robot_xacro_name',
            default_value='cer_R1SN003_arms_pos_only.urdf.xacro',
            description='The robot xacro file name')
    controllers_file_decl = DeclareLaunchArgument(
            'controllers_file',
            default_value='cer_R1SN003_ros2_control_arms_pos_only.yaml',
    ...
    right_controller_spawner = Node(
        ...
        executable="spawner",
        arguments=["right_arm_position_controller", "-c", "/controller_manager"],
    )

    left_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["left_arm_position_controller", "-c", "/controller_manager"],
    )
    ...
    return LaunchDescription(declared_args+[base_launch]+[right_controller_spawner,left_controller_spawner])
